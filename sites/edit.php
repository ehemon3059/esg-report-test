...
// Load site - verify it belongs to current company
$stmt = $pdo->prepare('SELECT * FROM sites WHERE id = :id AND company_id = :cid AND deleted_at IS NULL');
$stmt->execute([':id' => $siteId, ':cid' => company_id()]);
$site = $stmt->fetch();
...
<div class="max-w-xl mx-auto">
    <div class="mb-6 flex items-center space-x-3">
        ...
        <div>
            ...
            <p class="text-gray-500 text-base mt-0.5">Update site information</p>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        <form method="POST" class="space-y-5">
            <input type="hidden" name="id" value="<?= htmlspecialchars($siteId, ENT_QUOTES, 'UTF-8') ?>">

            <!-- Site Name -->
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Site Name <span class="text-red-500">*</span></label>
                <input type="text" id="name" name="name" required
                       value="<?= htmlspecialchars($_POST['name'] ?? $site['name'], ENT_QUOTES, 'UTF-8') ?>"
            ...
            <div>
                ...
                <textarea id="address" name="address" rows="3"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition resize-none"><?= htmlspecialchars($_POST['address'] ?? $site['address'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
            </div>
            ...
            <div>
                ...
                <input type="text" id="country" name="country"
                       value="<?= htmlspecialchars($_POST['country'] ?? $site['country'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition">
            </div>
